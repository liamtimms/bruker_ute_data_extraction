...
for i=1:s(1)
    ...
    if ~strcmp(MethodType, '<Bruker:EPI>')
        ...
        elseif strcmp(MethodType,'afi')
            
            AFI_Count=AFI_Count+1; % Used for savenametype2
            [~, AFINii1, AFINii2]=AFIextract_v2(pathname,scanNum,savenametype,AFI_Count);
            ScanOverview.AFIs.Niis(i,1)={AFINii1};
            ScanOverview.AFIs.Niis(i,2)={AFINii2};
            for j=1:1:2
                ParamName=sprintf('afi_TR%d=',j);
                TimeToRepetition = get_Param(FileString, dParamName);
                ScanOverview.AFIs.AfiTRs(i,j)={TimeToRepetition};
            end
        ...
        elseif strcmp(MethodType,'<Bruker:RARE>')
            ...
            Directions=str2num(SPackArrSliceOrient(Spaces(1)+1:Spaces(2)-1));
            if Directions==1
        ...
        elseif strcmp(MethodType, 'RAREVTR')
            %         [RAREvTRNii, T1Nii]=RAREvTRextract_v1(pathname,scanNum,savenametype,AFI_Count);
