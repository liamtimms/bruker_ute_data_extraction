function [nii, slopelineSplit] = ThreeDUTEextract_PV6_v2(pathname, scanNum, savenametype, UTE_Count, n, SubjectName)
%ThreeDUTEextract pre-processes 3D UTE scans from raw Bruker data File
...
for j=1:1:numRecos
    ...
    scanFileReco=strcat(pathname,'\',scanNum,'\pdata\',recoNum,'\reco'); %df
    ...
    % accumulative signal (i.e. S_tot=S_1+S_2+S_3...+S_N, where N is numAvg)
    % thus, Average Signal Intensity=S_tot/NumAvg
    Image=Image./NumAvg;
    ...
    if strcmp('COMPLEX_IMAGE',recoType)==1
        ...
        if savenametype==1
            %SaveName=strcat(pathsplit(1),StudyName,'_Scan',scanNum,'_UTE3D_',recoType,'.nii');
            
            NewScanNum=sprintf('%d',n);
            SaveName=strcat(SubjectName,'_Scan',NewScanNum,'_UTE3D_',recoType,'.nii');
        elseif savenametype==2
            counter=sprintf('%d',UTE_Count);
            SaveName=strcat(SubjectName,'_UTE3Dscan',counter,'_',recoType,'.nii');
        end
        nii.img = nii.img.*(SLOPE1/RG);
