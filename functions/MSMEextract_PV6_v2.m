function [nii, nii_TR1, nii_TR2] = MSMEextract_PV6_v2(pathname,scanNum, savenametype, MSME_Count, n,SubjectName, NSlices)
%MSMEextract pre-processes MSME scans from raw Bruker data File
%   pathname is the filepath with patient name
%   scanNum is the folder number containing MSME raw data from Bruker
%   savenametype : 1 for scan number, 2 for progression
%   Count : used for progression savenametype 2
%   example: MSMEextract('C:\...\Patientname.xyz','5',1,1)
...
% FOR EACH MSME RECO: Export scan and scale by RG and SLOPE
for j=1:1:numRecos
    recoNum=sprintf('%d',j);
    ...
    if savenametype==1
        ...
        SaveName=strcat(SubjectName,'_Scan',NewScanNum,'_MSME_',recoType,'.nii');
    elseif savenametype==2
        counter=sprintf('%d',MSME_Count);
        SaveName=strcat(SubjectName,'_MSMEscan',counter,'_',recoType,'.nii');
    end
